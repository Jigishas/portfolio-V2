...
/* ParallaxBackground component placed at bottom to avoid multiple imports scattering */
function ParallaxBackground() {
  const { scrollY } = useScroll();
  // small transform range for subtle vertical parallax
  const y = useTransform(scrollY, [0, 600], [0, -60]);
  const opacity = useTransform(scrollY, [0, 600], [0.12, 0.06]);
  ...
  return (
    <motion.div className="absolute inset-0 pointer-events-none" style={{ y, opacity }}>
      ...
      <motion.div
        ...
        style={{ opacity: useTransform(scrollY, [0, 800], [0.04, 0.25]) }}
      />
    </motion.div>
  );
}
